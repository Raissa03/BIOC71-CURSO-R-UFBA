...
is.vector() #É UMA VETOR? V OUF
is.numeric() # É UM NUMERO? V OU F
is.character()# É UM TEXTO?
is.matrix(peso) #É UMA MATRIZ
is.na() # OS VALORES SÃO NA?
is.data.frame() # É UM DATA.FRAME?
...
# mOSTRA TUDO O QUE OCORRE IGNORANDO AS REPETIÇÕES
...
sort(unique(caixeta.sem.tabebuia$especie)) #unique me cita as especies e sort é uma função que ordena

#agora crie um objeto removendo todas as "Tabebuia cassinoides" do local retiro
#(somente do local retiro) #(! é nao. não pegue o que cumpre as condições a seguir)
...
comNA=data.frame(id,c(a,b,c,d))
 semNA =comNA[!is.na(comNA$c.a..b..c..d.),] #is.NA MOSTRA COM true onde estão o na
semNA 
...
#if se isso por isso faça isso, else se não for faça aquilo
